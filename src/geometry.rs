...
pub fn objects_to_bbox<T: Bounded>(objects: &[T]) -> Option<BBox> {
    ...
    seen.then_some(BBox::new(x0, top, x1, bottom))
}
...
pub fn calculate_area(bbox: BBox) -> Result<f64> {
    if !bbox.is_valid() {
        return Err(Error::InvalidBBox(format!("invalid bbox dimensions: {:?}", bbox)));
    }
    Ok(bbox.area())
}
    ...
    F: Fn(&Edge) -> f64,
{
    ...
    let mut current_sum = key(&sorted[0]);
    let mut current_avg = current_sum;
    for edge in sorted.into_iter().skip(1) {
        let value = key(&edge);
        if (value - current_avg).abs() <= tolerance {
            current_sum += value;
            current.push(edge);
            current_avg = current_sum / current.len() as f64;
        } else {
            groups.push(current);
            current_sum = value;
            current_avg = value;
